...
EOD;
    ...
    {
        if (is_string($icon)) {
            //uncomment if wish to use natvie icon by bootstrap
            /*
    ...
    public static function dropdownToggleMenuLink($label, $url = '#', $htmlOptions = array(), $depth = 0)
    {
        ...
        //$htmlOptions['data-toggle'] = 'dropdown';
        //$htmlOptions['data-toggle'] = ''; // Remove data-toggle='dropdown' to support ACE admin
        return self::link($label, $url, $htmlOptions);
    }
     ...
     * @return string the generated menu.
     */
    public static function nav($type, $items, $htmlOptions = array(), $submenuHtmlOptions=array()) //add submenuHtmlOptions parameter inherited from TbNav.php
    {
        ...
        if (!empty($type)) {
            ...
            //self::addCssClass('', $htmlOptions); //If no type defined derieved class from htmlOptions
        }
        ...
        return self::menu($items, $htmlOptions, $submenuHtmlOptions);
    }
     ...
     * @return string the generated menu.
     */
    public static function menu(array $items, $htmlOptions = array(), $submenuHtmlOptions= array(), $depth = 0) //add submenuHtmlOptions parameter
    {
            ...
            foreach ($items as $itemOptions) {
                if (is_string($itemOptions)) {
                    ...
                    if (empty($items)) {
                        ...
                        $output .= self::menuDropdown($label, $url, $items, $itemOptions, $submenuHtmlOptions, $depth);
                    }
                }
            }
            $output .= '</ul>';
     ...
     */
    protected static function menuDropdown($label, $url, $items, $htmlOptions,$submenuHtmlOptions, $depth = 0) //add submenuHtmlOptions parameter
    {
        ...
        //self::addCssClass('dropdown-menu', $menuOptions);
         // adding for $submenuHtmlOptions 
        if (empty($submenuHtmlOptions)) {
            self::addCssClass('dropdown-menu', $menuOptions);
        } else {
            self::addCssClass($submenuHtmlOptions, $menuOptions);
        }
