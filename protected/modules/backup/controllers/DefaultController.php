...
class DefaultController extends Controller {
    ...
    public function accessRules() {
        return array(
            ...
            array('allow', // allow admin user to perform 'admin' and 'delete' actions
                'actions' => array('admin', 'delete', 'clean', 'index', 'view', 'create', 'upload', 'download', 'restore', 'SystemRestore'),
                'users' => array('admin'),
            ),
    ...
    public function getData($tableName) {
        ...
        /* Another technique from http://bit.ly/16Q5Zvj http://bit.ly/16Q5Yra */
        foreach ($dataReader as $row) {
            $data_string.= 'INSERT INTO ' . $tableName . ' VALUES(';
            foreach ($row as $data) {
    ...
    public function getTables($dbName = null) {
        // fixed to show only table otherwise it will issue the error undefined index Create Table
    ...
    public function actionCreate() {
        ...
        $message = '<stron>Succesfully</stron> created database backup file';
    ...
    public function actionSystemRestore($file = "system_restore.sql") {
        //$path = $this->path;
        //$files = glob($this->path."family_pos.sql");

        if (isset($file)) {
            $sqlFile = $this->path . $file;
            $message = 'Succesfully system restored database backup file:  ' . $file;
            $this->execSqlFile($sqlFile);
            Yii::app()->user->setFlash('success', $message);
        } else {
            $message = 'No files in directory';
